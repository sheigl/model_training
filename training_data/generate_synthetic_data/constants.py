...
REQUIREMENTS_BASE = [
    ...
    "CRITICAL — Trigger ordering: Before writing any answer, list the stack/trigger sequence in numbered order. Abilities on the stack resolve last-in-first-out. ETB triggers fire after a permanent enters. Static abilities (like Intruder Alarm) resolve immediately as state changes. If a card's ability triggers another card's ability, the triggered ability goes on the stack first, then the causing ability resolves. Your answer MUST describe events in the same order as the COMBO STEPS. A correct description of individual triggers in the wrong order is a factual error.",
    ...
    "Every answer MUST state the concrete outcome of the combo using the COMBO RESULT field. Do not use vague phrases like 'very powerful', 'wins the game', 'infinite triggers', 'infinite value', or 'overwhelm opponents' — state exactly what is produced (e.g. 'infinite damage', 'infinite mana of any color', 'infinite 1/1 Snake creature tokens').",
