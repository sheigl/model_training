...
print("You can now use 'interact.py' to generate text interactively!")
...
# Option 1: Generate from a prompt
print("\n" + "="*50)
print("GENERATING TEXT FROM PROMPT")
print("="*50)

# Your prompt here - change this to whatever you want!
prompt = "It is a truth universally acknowledged"
print(f"Prompt: {prompt}")

# Encode the prompt to get starting tokens
context = torch.tensor([encode(prompt)], dtype=torch.long, device=device)

# Generate 500 new tokens continuing from the prompt
...
# Option 2: Generate from scratch (no prompt)
print("\n" + "="*50)
print("GENERATING TEXT FROM SCRATCH")
print("="*50)

# Start with a single newline character (or any token)
context_blank = torch.zeros((1, 1), dtype=torch.long, device=device)

# Generate 500 new tokens
generated_blank = model.generate(context_blank, max_new_tokens=500)

print(decode(generated_blank[0].tolist()))