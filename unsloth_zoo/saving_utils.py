...
def _accelerator_is_available():
    """Check if any accelerator (CUDA or XPU) is available."""
...
def _get_accelerator_device():
    """Return the best available accelerator device string, or 'cpu'."""
    if torch.cuda.is_available(): return "cuda"
    if hasattr(torch, "xpu") and torch.xpu.is_available(): return "xpu"
    return "cpu"

def _accelerator_empty_cache():
    """Empty the cache on the available accelerator."""
    if torch.cuda.is_available():
        torch.cuda.empty_cache()
    elif hasattr(torch, "xpu") and torch.xpu.is_available():
        torch.xpu.empty_cache()

def _accelerator_synchronize():
    """Synchronize the available accelerator."""
    if torch.cuda.is_available():
        torch.cuda.synchronize()
    elif hasattr(torch, "xpu") and torch.xpu.is_available():
        torch.xpu.synchronize()

def _tensor_is_on_accelerator(tensor):
    """Check if a tensor is on an accelerator device (CUDA or XPU)."""
    if tensor.is_cuda: return True
    if hasattr(tensor, "is_xpu") and tensor.is_xpu: return True
    return False
try:
...
def _merge_moe_gate_expert(gate_W, lora_stats, expert_idx, num_experts, output_dtype):
    ...
    try:
        ...
        device = gate_W.device if _tensor_is_on_accelerator(gate_W) else _get_accelerator_device()
...
def _merge_moe_up_expert(up_W, lora_stats, expert_idx, num_experts, output_dtype):
    ...
    try:
        ...
        device = up_W.device if _tensor_is_on_accelerator(up_W) else _get_accelerator_device()
...
def _merge_moe_down_proj_expert(down_W, lora_stats, expert_idx, num_experts, output_dtype):
    ...
    try:
        ...
        device = down_W.device if _tensor_is_on_accelerator(down_W) else _get_accelerator_device()
...
def _merge_moe_fused_gate_up_expert(gate_up_W, lora_stats, output_dtype):
    ...
    try:
        ...
        device = (
            gate_up_W.device
            if _tensor_is_on_accelerator(gate_up_W)
            else _get_accelerator_device()
        )
...
def _merge_moe_fused_down_proj_expert(down_W, lora_stats, output_dtype):
    ...
    try:
        ...
        device = (
            down_W.device
            if _tensor_is_on_accelerator(down_W)
            else _get_accelerator_device()
        )
    ...
    with safe_open(filename_original, framework = "pt", device = "cpu") as file: # Open original file for reading
        ...
        for key in safetensor_keys:
            ...
            if key.endswith("_blocks"):
                ...
                if _accelerator_is_available():
                    _accelerator_synchronize()  # Wait for previous operations to complete
                    _accelerator_empty_cache()
...
def merge_and_overwrite_lora(
    ...
    import subprocess
    is_t4 = False if (hasattr(torch, "xpu") and torch.xpu.is_available()) else ("Tesla T4" in str(torch.cuda.get_device_name(0)) if torch.cuda.is_available() else False)
...
def get_memory_stats():
    ...
    # GPU Memory (for each GPU - CUDA or XPU)
    stats['gpus'] = []
    if torch.cuda.is_available():
        ...
        for i in range(torch.xpu.device_count()):
            gpu_mem = torch.xpu.mem_get_info(i)
            total = gpu_mem[1]
            free = gpu_mem[0]
            stats['gpus'].append({
                'device_id': i,
    ...
    for gpu in stats['gpus']:
        ...
        if chunk_size:
            ...
            suitable_strategies.append({
                'device_type': gpu.get('device_type', _get_accelerator_device()),
