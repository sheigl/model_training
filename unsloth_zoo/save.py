...
def unsloth_save_model(
    ...
    _accel_device = _get_accelerator_device()
    if _accel_device == "cuda":
        max_vram = int(torch.cuda.get_device_properties(0).total_memory * maximum_memory_usage)
    elif _accel_device == "xpu":
        max_vram = int(torch.xpu.get_device_properties(0).total_memory * maximum_memory_usage)
    else:
        max_vram = 0
    ...
    for j, layer in enumerate(ProgressBar(internal_model.model.layers)):
        for item in LLAMA_WEIGHTS:
            ...
            _mem_allocated = torch.cuda.memory_allocated() if _accel_device == "cuda" else (torch.xpu.memory_allocated() if _accel_device == "xpu" else float("inf"))
            if (_mem_allocated + W.nbytes) < max_vram:
                # Save to GPU memory
                state_dict[name] = W
...
def save_to_gguf(
    ...
    _bf16_ok = torch.cuda.is_bf16_supported() if torch.cuda.is_available() else (True if hasattr(torch, "xpu") and torch.xpu.is_available() else False)
    if model_dtype == "bf16" and not _bf16_ok:
        logger.warning(
    ...
    _bf16_ok2 = torch.cuda.is_bf16_supported() if torch.cuda.is_available() else (True if hasattr(torch, "xpu") and torch.xpu.is_available() else False)
    if first_conversion == "bf16" and not _bf16_ok2:
